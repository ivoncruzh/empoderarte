...
app.get('/api/monthly',auth,async(q,s)=>{try{const r=await pool.query(`SELECT id,matricula,name,last_name,phone,plan,monthly_fee,due_date,status,benefit_level FROM students ORDER BY due_date NULLS LAST,id DESC`);const now=new Date();s.json(r.rows.map(x=>{const d=x.due_date?new Date(x.due_date+'T12:00:00'):null;const days=d?Math.ceil((d-now)/86400000):null;return {...x,payment_status:!d?'Sin fecha':days<0?'Vencido':days<=7?'Próximo':'Al corriente',days_remaining:days}}))}catch(e){console.error(e);s.status(500).json({error:'No se pudieron cargar las mensualidades'})}});
...
app.post('/api/teacher-attendance',auth,async(q,s)=>{try{const b=q.body||{},t=await pool.query('SELECT * FROM teachers WHERE id=$1',[b.teacher_id]);if(!t.rowCount)return s.status(404).json({error:'Maestro no encontrado'});if(!b.entry_time&&!b.exit_time)return s.status(400).json({error:'Registra entrada o salida'});const r=await pool.query('INSERT INTO teacher_attendance(teacher_id,attended_at,entry_time,exit_time,discipline,notes) VALUES($1,$2,$3,$4,$5,$6) RETURNING *',[b.teacher_id,b.attended_at||today(),b.entry_time||null,b.exit_time||null,b.discipline||t.rows[0].discipline||'',b.notes||'']);await audit(q.user,'CREATE','ASISTENCIA_MAESTROS',r.rows[0].id,'Asistencia '+t.rows[0].matricula);s.status(201).json(r.rows[0])}catch(e){console.error(e);s.status(500).json({error:'No se pudo registrar la asistencia del maestro'})}});
...
app.get('/api/locker-plans',auth,async(q,s)=>{try{s.json((await pool.query('SELECT * FROM locker_plans WHERE active=1 ORDER BY months,id')).rows)}catch(e){console.error(e);s.status(500).json({error:'No se pudieron cargar los planes de lockers'})}});
...
app.post('/api/lockers/:id/penalty',auth,async(q,s)=>{try{const b=q.body||{},amount=Number(b.amount);if(!amount)return s.status(400).json({error:'Monto inválido'});const r=await pool.query('INSERT INTO locker_penalties(locker_id,amount,reason) VALUES($1,$2,$3) RETURNING *',[q.params.id,amount,b.reason||'Penalización']);await audit(q.user,'CREATE','PENALIZACIONES_LOCKERS',r.rows[0].id,b.reason||'Penalización');s.status(201).json(r.rows[0])}catch(e){console.error(e);s.status(500).json({error:'No se pudo registrar la penalización'})}});
