...
app.get('/api/students',auth,async(q,s)=>{try{const x=(q.query.q||'').trim();const params=[];let where='';if(x){params.push(`%${x}%`,`%${x}%`,`%${x}%`);where='WHERE matricula ILIKE $1 OR name ILIKE $2 OR last_name ILIKE $3';}const page=Math.max(0,Number(q.query.page||0)||0);const pageSize=10;const limitOffset=x?'':'LIMIT $4 OFFSET $5';if(!x){params.push(pageSize,page*pageSize);}const r=await pool.query(`SELECT id,matricula,name,last_name,birth_date,phone,email,tutor,tutor_phone,plan,status,enrollment_date,monthly_fee,due_date,benefit_level,notes,photo_data,plan_id,discipline_id,group_name,plan_start_date,plan_end_date,promotion_id,created_at,updated_at,(photo_data IS NOT NULL AND photo_data <> '') AS has_photo FROM students ${where} ORDER BY id DESC ${limitOffset}`,params);const lim=Date.now()+7*864e5;const now=new Date();const out=r.rows.map(a=>{const due=a.due_date||null;let status='Sin fecha';if(due&&a.status==='Activo'){const d=new Date(due+'T12:00:00');status=d<now?'Vencido':d.getTime()<=lim?'Próximo':'Al corriente';}return {...a,due_date:due,payment_status:status};});s.json(out)}catch(e){console.error(e);s.status(500).json({error:'No se pudieron cargar los alumnos'})}});
