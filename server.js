...
app.get('/api/students',auth,async(q,s)=>{try{const x=(q.query.q||'').trim();const r=x?await pool.query('SELECT * FROM students WHERE matricula ILIKE $1 OR name ILIKE $2 OR last_name ILIKE $3 ORDER BY id DESC',[`%${x}%`,`%${x}%`,`%${x}%`]):await pool.query('SELECT * FROM students ORDER BY id DESC');const lim=Date.now()+7*864e5;const out=await Promise.all(r.rows.map(async a=>{let due=a.due_date||null,status='Sin fecha';try{const charges=await ensureMonthlyCharges(a.id,12);const open=charges.filter(c=>Number(c.balance)>0);const current=open[0]||charges[charges.length-1];due=current?.due_date||due;if(due&&a.status==='Activo'){const d=new Date(due+'T12:00:00');status=current?.status==='Pagado'?'Pagado':d<new Date()?'Vencido':d.getTime()<=lim?'Próximo':'Al corriente';}}catch(_){if(due&&a.status==='Activo'){const d=new Date(due+'T12:00:00');status=d<new Date()?'Vencido':d.getTime()<=lim?'Próximo':'Al corriente';}}return {...a,due_date:due,payment_status:status};}));s.json(out)}catch(e){console.error(e);s.status(500).json({error:'No se pudieron cargar los alumnos'})}});
