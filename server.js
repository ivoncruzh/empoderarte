...
async function createPerformanceIndexes(){await pool.query('CREATE INDEX IF NOT EXISTS idx_students_name ON students (LOWER(name), LOWER(last_name))');await pool.query('CREATE INDEX IF NOT EXISTS idx_students_matricula ON students (matricula)');await pool.query('CREATE INDEX IF NOT EXISTS idx_monthly_charges_student_period ON monthly_charges (student_id, period_start)');await pool.query('CREATE INDEX IF NOT EXISTS idx_payment_allocations_charge ON payment_allocations (charge_id)');}
async function init(){
...
app.get('/api/dashboard',auth,async(q,s)=>{try{const [total,active,overdue,upcoming,revenue,attendance,exp,recent,teachers]=await Promise.all([
 pool.query('SELECT COUNT(*) c FROM students'),pool.query("SELECT COUNT(*) c FROM students WHERE status='Activo'"),pool.query("SELECT COUNT(*) c FROM students WHERE status='Activo' AND due_date IS NOT NULL AND due_date < CURRENT_DATE::text"),pool.query("SELECT COUNT(*) c FROM students WHERE status='Activo' AND due_date IS NOT NULL AND due_date >= CURRENT_DATE::text AND due_date <= (CURRENT_DATE+7)::text"),pool.query("SELECT COALESCE(SUM(amount),0) total FROM payments WHERE paid_at >= date_trunc('month',CURRENT_DATE)::date::text"),pool.query("SELECT COUNT(*) c FROM attendance WHERE attended_at=CURRENT_DATE::text AND status='Presente'"),pool.query("SELECT * FROM students WHERE status='Activo' AND due_date IS NOT NULL AND due_date <= (CURRENT_DATE+7)::text ORDER BY due_date LIMIT 8"),pool.query('SELECT p.*,s.matricula,s.name,s.last_name FROM payments p JOIN students s ON s.id=p.student_id ORDER BY p.id DESC LIMIT 8'),pool.query("SELECT COUNT(*) c FROM teachers WHERE status='Activo'")]);s.json({total:+total.rows[0].c,active:+active.rows[0].c,overdue:+overdue.rows[0].c,upcoming:+upcoming.rows[0].c,revenue:+revenue.rows[0].total,attendance:+attendance.rows[0].c,exp:exp.rows,recent:recent.rows,teachers:+teachers.rows[0].c})}catch(e){console.error(e);s.status(500).json({error:'No se pudo cargar el panel'})}});
app.get('/api/students',auth,async(q,s)=>{try{const x=(q.query.q||'').trim();const fields='id,matricula,name,last_name,phone,plan,status,enrollment_date,monthly_fee,due_date,new_income,plan_id,discipline_id,group_name';const r=x?await pool.query(`SELECT ${fields} FROM students WHERE matricula ILIKE $1 OR name ILIKE $2 OR last_name ILIKE $3 ORDER BY id DESC LIMIT 100`,[`%${x}%`,`%${x}%`,`%${x}%`]):await pool.query(`SELECT ${fields} FROM students ORDER BY id DESC LIMIT 100`);const lim=Date.now()+7*864e5;s.json(r.rows.map(a=>({...a,payment_status:a.due_date&&a.status==='Activo'?(new Date(a.due_date+'T12:00:00')<new Date()?'Vencido':new Date(a.due_date+'T12:00:00').getTime()<=lim?'Próximo':'Al corriente'):'Sin fecha'})))}catch(e){console.error(e);s.status(500).json({error:'No se pudieron cargar los alumnos'})}});
...
app.get('/api/monthly',auth,async(q,s)=>{try{const r=await pool.query(`WITH charge_totals AS (SELECT mc.*,COALESCE(SUM(pa.amount),0) paid_amount FROM monthly_charges mc LEFT JOIN payment_allocations pa ON pa.charge_id=mc.id GROUP BY mc.id), ranked AS (SELECT ct.*,ROW_NUMBER() OVER (PARTITION BY student_id ORDER BY (CASE WHEN GREATEST(0,ct.amount-ct.discount-ct.paid_amount)>0 THEN 0 ELSE 1 END),ct.period_start) rn FROM charge_totals ct), chosen AS (SELECT * FROM ranked WHERE rn=1) SELECT st.id,st.matricula,st.name,st.last_name,st.phone,st.plan,st.monthly_fee,st.due_date,st.status,st.benefit_level,c.id charge_id,c.period_start charge_period,COALESCE(c.amount,st.monthly_fee) charge_amount,COALESCE(GREATEST(0,c.amount-c.discount-c.paid_amount),0) charge_balance,COALESCE(c.due_date,st.due_date) effective_due_date FROM students st LEFT JOIN chosen c ON c.student_id=st.id ORDER BY COALESCE(c.due_date,st.due_date) NULLS LAST,st.id DESC`);const now=Date.now(),lim=now+7*864e5;const out=r.rows.map(x=>{const due=x.effective_due_date;const d=due?new Date(due+'T12:00:00').getTime():null;const days=d===null?null:Math.ceil((d-now)/86400000);const paid=Number(x.charge_balance||0)<=0&&x.charge_id;return {...x,payment_status:!d?'Sin fecha':paid?'Pagado':days<0?'Vencido':d<=lim?'Próximo':'Al corriente',days_remaining:days};});s.json(out)}catch(e){console.error(e);s.status(500).json({error:'No se pudieron cargar las mensualidades'})}});
...
app.get('/api/teachers',auth,async(q,s)=>{try{s.json((await pool.query('SELECT * FROM teachers ORDER BY id DESC')).rows)}catch(e){console.error(e);s.status(500).json({error:'No se pudieron cargar los maestros'})}});
...
app.post('/api/personal-attendance',auth,async(q,s)=>{try{
  ...
  if(!isDirector){
    ...
    if(action==='entrada'){
      ...
      if(open.rowCount)return s.status(400).json({error:'La entrada de hoy ya está registrada; registra la salida.'});
      entry=mexicoTime();late=Math.max(0,timeToMin(entry)-9*60);
    } else if(action==='salida'){
...
app.get('/api/payroll',auth,director,async(q,s)=>{try{
  ...
  const teachers=await pool.query(`SELECT t.id,t.name,t.last_name,t.rate_hour,
    ...
    FROM teachers t LEFT JOIN teacher_attendance a ON a.teacher_id=t.id AND a.attended_at>=date_trunc('month',CURRENT_DATE)::date::text
    LEFT JOIN LATERAL (SELECT start_time,end_time FROM schedules s WHERE s.teacher_id=t.id AND s.active=1 AND s.day_of_week=EXTRACT(DOW FROM a.attended_at::date)::int LIMIT 1) sc ON true GROUP BY t.id`);
  teachers.rows.forEach(x=>{const gross=Number(x.hours||0)*Number(x.rate_hour||0),ded=Number(x.absence_deduction||0)+Number(x.late_deduction||0);rows.push({type:'Maestro',name:x.name,last_name:x.last_name,hours:Number(x.hours||0),gross,total:Math.max(0,gross-ded),absence_deduction:Number(x.absence_deduction||0),late_deduction:Number(x.late_deduction||0),deduction:ded})});
  const people=await pool.query(`SELECT p.id,p.name,p.last_name,p.rate_hour,
    COALESCE(SUM(CASE WHEN a.entry_time IS NOT NULL AND a.exit_time IS NOT NULL THEN EXTRACT(EPOCH FROM (('2000-01-01 '||a.exit_time)::timestamp-('2000-01-01 '||a.entry_time)::timestamp))/3600 ELSE 0 END),0) hours,
    COALESCE(SUM(CASE WHEN a.status='No asistió' THEN 8*p.rate_hour ELSE 0 END),0) absence_deduction,
    COALESCE(SUM(CASE WHEN a.late_minutes>0 THEN a.late_minutes*p.rate_hour/60 ELSE 0 END),0) late_deduction
    FROM personal p LEFT JOIN personal_attendance a ON a.personal_id=p.id AND a.attended_at>=date_trunc('month',CURRENT_DATE)::date::text GROUP BY p.id`);
  people.rows.forEach(x=>{const gross=Number(x.hours||0)*Number(x.rate_hour||0),ded=Number(x.absence_deduction||0)+Number(x.late_deduction||0);rows.push({type:'Personal',name:x.name,last_name:x.last_name,hours:Number(x.hours||0),gross,total:Math.max(0,gross-ded),absence_deduction:Number(x.absence_deduction||0),late_deduction:Number(x.late_deduction||0),deduction:ded})});
  s.json(rows);
